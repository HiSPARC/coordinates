...
When solar time and UTC time are starting to get out of sync a leap
second is added to UTC time. This means that a second is counted twice.
The ...[USNO?] decides when to add leap seconds. \hisparc does not use
UTC time because seconds are repeated which makes it possible for a
station to detect two events at the same 'time'. Moreover it interferes
when looking for coincidences between stations.
